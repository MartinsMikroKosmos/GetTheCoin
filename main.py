...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        player_image = str(
            Path.cwd() / "Assets" / "images" / "Dead.png"
        )
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        coin_path = str(Path.cwd() / "Assets" / "images" / "coin_gold.png")
        ...
        coin_images = [coin_sheet.subsurface((i * frame_width, 0, frame_width, frame_height)) for i in range(8)]
        self.animation_frames = cycle(coin_images)

        self.surf = next(self.animation_frames)
        ...
        # Setze einen Timer für die Animation
        self.animation_timer = pygame.time.get_ticks()

    def update(self):
        now = pygame.time.get_ticks()
        if now - self.animation_timer > 100:  # Adjust the duration between frames
            self.surf = next(self.animation_frames)
...
coin_pickup_sound = pygame.mixer.Sound(
    str(Path.cwd() / "Assets" / "sounds" / "coin_pickup.mp3")
)
